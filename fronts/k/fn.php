...
/**
 * Method:ufvtest() URL地址
 * Parameter:url(string) -- 地址
 */
if ( !function_exists('ufvtest') ) {
	function ufvtest($url) {
		if ( ONLINE ) return 'http://crwy.edisonluorui.com' . $url . '?v=' . VERSION;
		else return 'http://resource.crwy' . $url . '?v=' . VERSION;
	}
}
...
if ( !function_exists('ujv') ) {
	function ujv($url) {
		if ( ONLINE ) return 'http://crwy.edisonluorui.com' . $url . '?v=' . VERSIONJS;
		else return 'http://resource.crwy' . $url . '?v=' . VERSIONJS;
	}
}
/**
 * Method:rfv() URL地址
...
/**
 * Method:api_success2() 接口成功
 * Parameter:data(array) -- 返回数据
 */
if ( !function_exists('api_success2') ) {
	function api_success2($data = array()) {
		$json = array(
			'status' => 2000,
			'data' => $data
		);
...
/**
 * Method:api_error2() 接口报错
 * Parameter:msg(string) -- 错误提示语
 */
if ( !function_exists('api_error2') ) {
	function api_error2($code = 0,$msg = '') {
		$json = array(
			'status' => $code,
			'msg' => $msg
		);
	...
	function json_encode2($arr = array()) {
		...
		if ( $arr ) {
			foreach ($arr as $key => $item) {
				...
				elseif ( is_string($item) ) $item = '"' . addslashes($item) . '"';
				elseif ( $item === '' ) $item = '""';
...
if ( !function_exists('upfile') ) {
	function upfile($key = '',$allow = array(),$maxsize = 0) {
		...
		if ( SYNC && ONLINE ) {
			require_once './k/package/ftp.php';
			foreach ($config['servers'] as $server) {
				...
				@$ftp->mkdir('/' . NAME,0777);
				@$ftp->mkdir('/' . NAME . '/upload',0777);
				@$ftp->mkdir('/' . NAME . '/upload/' . date('Y'),0777);
				@$ftp->mkdir('/' . NAME . '/upload/' . date('Y/md'),0777);
				$ftp->upload('.' . $file_path,'/' . NAME . $file_path);
				$ftp->close();
			}
		}
		return $file_path;
