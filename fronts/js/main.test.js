...
function ufv(url) {
	if ( ONLINE ) return "http://fronts.edisonluorui.com" + url + "?v=" + VERSION;
	else return "http://resource.crwy" + url + "?v=" + VERSION;
}
function ucv(url) {
	if ( ONLINE ) return "http://fronts.edisonluorui.com" + url;
...
$.extend( {
	...
	https : function(path,data,success,error,time) {
		...
		var domain = ONLINE ? "https://apis.edisonluorui.com:34433" : "http://192.168.1.21:6106";
		var ajax = $.ajax( {
			type : "POST",
			url : domain + path,
			dataType : "json",
	...
	httpsPay : function(data,success,error) {
		...
		var ajax = $.ajax( {
			type : "POST",
			//url : "https://apis.edisonluorui.com/order",
			url : "https://apis.edisonluorui.com:34433/wftpay",
	...
	httpsSns : function(data,success,error) {
		...
		var ajax = $.ajax( {
			type : "POST",
			url : "https://apis.edisonluorui.com/sns/sendcode",
			dataType : "json",
			data : data,
			success : function(callback) {
				if ( success ) success(callback);
