...
function ufv(url) {
	if ( ONLINE ) return "https://bestcdn.szhlsg.com" + url + "?v=" + VERSION;
	else return "http://resource.best" + url + "?v=" + VERSION;
}
function ufv2base(url) {
	if ( ONLINE ) return url.replace("https://bestcdn.szhlsg.com","").split("?")[0];
	else return url.replace("http://resource.best","").split("?")[0];
}
function ucv(url) {
	if ( ONLINE ) return "https://best.szhlsg.com" + url + "?v=" + VERSION;
	else return "http://best" + url + "?v=" + VERSION;
}
...
/*弹出提示模块*/
	...
	var loading = {
		show : function() {
			var html = '<div style="display:block;position:fixed;z-index:1001100;top:0;right:0;bottom:0;left:0;background-color:rgba(0,0,0,0.5);"></div>';
...
$.fn.extend( {
	...
	tap : function(fn,ontap,over,overfn,upfn) {
		if ( fn ) {
			var that = this;
			if ( isMobile && (window.OPEN_DATA.pf == "weixin" || window.OPEN_DATA.platform == 1 ) ) {
				this.swiping( {
					start : function() {
	...
	//取消触碰
	untap : function() {
		if ( isMobile && (window.OPEN_DATA.pf == "weixin" || window.OPEN_DATA.platform == 1 ) ) this.unbind("touchstart").unbind("touchmove").unbind("touchend");
