...
if ( !function_exists('ufv') ) {
	function ufv($url) {
		if ( ONLINE ) return 'https://crwy-gm.edisonluorui.com' . $url . '?v=' . VERSION;
		else return 'http://resource.mf' . $url . '?v=' . VERSION;
	}
}
/**
 * Method:ujv() URL地址
	...
	function json_encode2($arr = array()) {
		...
		if ( $arr ) {
			foreach ($arr as $key => $item) {
				...
				elseif ( is_string($item) ) $item = '"' . str_replace('"','\"',$item) . '"';
				elseif ( $item === '' ) $item = '""';
	...
	function upfile($key = '',$allow = array(),$maxsize = 0) {
		...
		if ( SYNC && ONLINE ) {
			...
			foreach ($config['servers'] as $server) {
				...
				$ftp->upload('.' . $file_path,$file_path);
				$ftp->close();
			}
		}
		return $file_path;
