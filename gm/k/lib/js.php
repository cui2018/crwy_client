...
class Js {
	...
	public function jquery($version = '1.7.2') {
		...
		else echo '<script src="http://resource.mf/'. (ROOT ? '' : PROJECT . '/') . 'js/jquery-' . $version . '.min.js"></script>' . "\r\n";
	}
	//Vue
	public function vue() {
		if ( ONLINE ) echo '<script src="' . ujv('/js/vue.min.js') . '"></script>' . "\r\n";
		else echo '<script src="http://resource.mf/'. (ROOT ? '' : PROJECT . '/') . 'js/vue.min.js"></script>' . "\r\n";
	}
	//main
	public function main() {		
		...
		else echo '<script src="http://resource.mf/'. (ROOT ? '' : PROJECT . '/') . 'js/main.js?v=' . VERSION . '"></script>' . "\r\n";
	}
	//手机号验证
	public function phone() {
		if ( ONLINE ) echo '<script src="' . ujv('/js/phone.js') . '"></script>' . "\r\n";
		else echo '<script src="http://resource.mf/'. (ROOT ? '' : PROJECT . '/') . 'js/phone.js?v=' . VERSION . '"></script>' . "\r\n";
	}
}